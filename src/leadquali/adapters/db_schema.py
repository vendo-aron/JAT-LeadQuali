...
PERSONAL_DATA_COLUMNS: dict[tuple[str, str], str] = {
    ...
    ("stripe_events", "payload"): (
        "a verified Stripe webhook body, stored verbatim (#35). An invoice object carries "
        "the billing contact's name, email and postal address. **A different data subject "
        "from every other entry here**: the customer's own billing contact, not an inbound "
        "lead — which is why its retention answer is the opposite shape. See "
        "leadquali.app.retention.COLUMN_DISPOSITION."
    ),
}
