...
SECRET_SOURCES = {
    ...
    # Both come from the Stripe dashboard, so neither can be generated by a template
    # (#34's runbook makes them by hand, like the Anthropic key). They are still required
    # parameters with no default: a stack that is not billing anybody passes the ARNs of
    # empty secrets, and whether the billing functions exist at all is decided by
    # StripePriceId, which is not a secret.
    "StripeApiKeySecretArn": None,
    "StripeWebhookSecretArn": None,
}
...
def test_a_function_with_the_database_secret_gets_the_parts_to_assemble_a_url(
    ...
    for logical_id, properties in _functions(application).items():
        ...
        checked += 1
    assert checked == FUNCTIONS_WITH_SECRETS, "every one of them talks to Postgres"
